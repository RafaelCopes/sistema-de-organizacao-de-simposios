...
import {
  createSymposiumSchema,
  updateSymposiumSchema,
  idSchema,
} from "../zodSchemas"; // Assuming you've created these schemas
...
export const createSymposium = async (req: Request, res: Response) => {
  ...
  if (!parsedBody.success) {
    return res.status(400).json({
      message: "Invalid symposium data",
...
export const updateSymposium = async (req: Request, res: Response) => {
  ...
  if (!parsedBody.success) {
    return res
      .status(400)
      .json({ message: "Invalid body data", errors: parsedBody.error.errors });
  }
  ...
  try {
    ...
    if (symposium.organizerId !== userId) {
      return res.status(403).json({
        message: "Access denied. Only the organizer can update this symposium.",
      });
    }
...
export const deleteSymposium = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (symposium.organizerId !== userId) {
      return res.status(403).json({
        message: "Access denied. Only the organizer can delete this symposium.",
      });
    }
