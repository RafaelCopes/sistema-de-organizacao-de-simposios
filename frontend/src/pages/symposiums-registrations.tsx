...
export function SymposiumRegistrations() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchSymposiumRegistrations = async () => {
      try {
        const symposiumResponse = await client.get(`/symposiums/${id}`);
        setSymposium(symposiumResponse.data);

        setRegistrations(symposiumResponse.data.registrations);
      } catch (error) {
        console.error("Error fetching symposium registrations:", error);
      } finally {
        setLoading(false);
      }
    };

    fetchSymposiumRegistrations();
  }, [id, user]);

  const handleAccept = async (registrationId) => {
    try {
      await client.put(
        `/symposiums/${symposium.id}/registrations/${registrationId}`,
        {
            status: "accepted",
        },
        {
        ...
        {symposium ? (
          <Box>
            ...
            >
              ...
              >
                Solicitações de Inscrição para {symposium.name}
              </Typography>
            </Box>
